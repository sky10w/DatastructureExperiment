...
void gameboard::roundbegin()
{
    ...
    setanimation(QString("://res/roundbegin.png"),
                 3000,
...
BuffView::BuffView()
{
    buffview.setSceneRect(0, 0, 260, 60);
}
...
void CardView::mouseReleaseEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    itembelow = dynamic_cast<EntityView *>(scene()->itemAt(event->scenePos(), QTransform()));
    ...
    auto distance = QLineF(curposx + 120,
                           curposy + 120,
                           event->scenePos().x(),
                           event->scenePos().y())
                        .length();
    ...
    if (distance < 200)
        ...
        deny();
    else if (info.targetType == info.ONE && (itembelow == nullptr || itembelow->type != "enemy"))
        deny();
