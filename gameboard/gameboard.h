...
#define MyDebug                                                                \
  qDebug() << "[In function" << (__FUNCTION__ + QString("]:")).toLatin1().data()
...
public:
  ...
  void setanimation(QPixmap pixmap, int time, qreal posx, qreal posy);

  void AnimationHurt(EntityView *entity);
signals:
  void roundover();
