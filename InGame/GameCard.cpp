...
QHash<QString, CardInfo> CardSystem::InitCards()
{
    ...
    for (auto i : array)
    {
        ...
        if (!obj.contains("uuid") ||
            ...
            !obj.contains("action"))
        {
            qWarning() << "Warning: invalid json format - " << "a card doesn't have attribute \"uuid\"";
        }
        CardInfo tempInfo;
        ...
        tempInfo.targetType = obj["target_type"].toInt();
        tempInfo.energy = obj["energy"].toInt();
        tempInfo.action = obj["action"].toString();
